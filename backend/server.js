...
// ========== Face-lock prefix — ใส่ทุก prompt เพื่อรักษาใบหน้า ==========
const FACE_LOCK =
    'CRITICAL INSTRUCTION: This is image-to-image transformation. ' +
    'You MUST preserve the EXACT same face, facial features, skin tone, ' +
    'eye shape, nose, mouth, jawline, and identity of the person in the reference image. ' +
    'The person must be instantly recognizable as the same individual. ' +
    'Change ONLY the costume/outfit and the background scenery. ' +
    'Do NOT alter face structure, facial proportions, or identity in any way. ';

const NEGATIVE_PROMPT =
    'deformed, blurry, bad anatomy, disfigured, ugly, low quality, watermark, text, logo, ' +
    'different person, wrong face, face change, new face, altered face, ' +
    'different identity, morphed face, distorted face';
...
app.post('/api/generate-leonardo', async (req, res) => {
    try {
        ...
        // Step 2: สร้าง prompt โดยใส่ FACE_LOCK ก่อนเสมอ
        const imagePrompt = FACE_LOCK + (prompt || '') +
            ' Professional lighting, cinematic, ultra realistic, 4k, sharp focus, detailed.';
        ...
        const payload = {
            ...
            init_strength: parseFloat(strength) // 0.45 — รักษาหน้าได้ดี ยังเปลี่ยนชุด+พื้นหลังได้
        };
